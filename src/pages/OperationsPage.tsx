...
function MaterialsTable({ props }: { props: OperationsPageProps }) {
  ...
  const [visionDetailFile, setVisionDetailFile] = useState<File | null>(null);
  ...
  const [visionCategorySlug, setVisionCategorySlug] = useState('all');
  const [visionUnit, setVisionUnit] = useState('шт');
  const [visionAvailability, setVisionAvailability] = useState('склад');
  const [visionCity, setVisionCity] = useState('Алматы');
  const [visionSupplier, setVisionSupplier] = useState('');
  const [visionPurchasePrice, setVisionPurchasePrice] = useState('');
  const [visionPurchaseCurrency, setVisionPurchaseCurrency] = useState('KZT');
  const [visionSalePrice, setVisionSalePrice] = useState('');
  const [visionSaleCurrency, setVisionSaleCurrency] = useState('KZT');
  const [visionMarginPercent, setVisionMarginPercent] = useState('');
  const [visionStatus, setVisionStatus] = useState('draft');
  const [visionLanguages, setVisionLanguages] = useState<string[]>(['RU']);
  const [visionSeoTitle, setVisionSeoTitle] = useState('');
  const [visionSeoDescription, setVisionSeoDescription] = useState('');
  ...
  const [visionDetailPreviewUrl, setVisionDetailPreviewUrl] = useState<string | null>(null);
  const content = props.almabuildContent;
  useEffect(() => {
    if (!visionFile) {
  ...
  useEffect(() => {
    if (!visionDetailFile) { setVisionDetailPreviewUrl(null); return undefined; }
    const url = URL.createObjectURL(visionDetailFile);
    setVisionDetailPreviewUrl(url);
    return () => URL.revokeObjectURL(url);
  }, [visionDetailFile]);
  ...
  const visionCategory = visionCategorySlug === 'all' ? null : content.materialCategories.find((item) => item.slug === visionCategorySlug) || null;
  ...
  async function runVision() {
    ...
    try {
      const response = await generateAlmabuildMaterialsFromPhoto({
        ...
        detailImage: visionDetailFile,
        price: visionPrice,
        categorySlug: visionCategory?.slug || '',
        categoryTitle: visionCategory?.title || 'Все',
        unit: visionUnit,
        availability: visionAvailability,
        city: visionCity,
        supplier: visionSupplier,
        purchasePrice: visionPurchasePrice,
        purchaseCurrency: visionPurchaseCurrency,
        salePrice: visionSalePrice,
        saleCurrency: visionSaleCurrency,
        marginPercent: visionMarginPercent,
        status: visionStatus,
        languages: visionLanguages,
        seoTitle: visionSeoTitle,
        seoDescription: visionSeoDescription
      });
  ...
  async function saveVisionDrafts() {
    ...
    try {
      ...
      setVisionDrafts([]);
      setVisionFile(null);
      setVisionDetailFile(null);
      setVisionPreviewUrl(null);
      setVisionDetailPreviewUrl(null);
  ...
  return <section className="ops-panel catalog-browser">
    ...
    {creating ? <KazaxbudMaterialEditorModal content={content} onClose={() => setCreating(false)} onRefresh={props.onRefresh} visionPanel={<Card className="bg-zinc-950"><CardHeader><CardTitle>Gemini Vision по фото</CardTitle></CardHeader><CardContent className="grid gap-3"><div className="grid gap-2"><span className="text-xs font-black text-zinc-400">Категория в базе</span><select className="h-10 rounded-md border border-zinc-800 bg-zinc-950 px-3 text-sm font-bold text-zinc-50" value={visionCategorySlug} onChange={(event) => setVisionCategorySlug(event.target.value)}><option value="all">Все</option>{content.materialCategories.map((category) => <option key={category.slug} value={category.slug}>{category.title}</option>)}</select></div><div className="grid gap-2"><span className="text-xs font-black text-zinc-400">Цена</span><Input value={visionPrice} onChange={(event) => setVisionPrice(event.target.value)} placeholder="например: от 3 500 ₸ / лист" /></div><div className="grid grid-cols-2 gap-2"><label className="grid gap-1"><span className="text-xs font-black text-zinc-400">Ед. изм.</span><select className="h-10 rounded-md border border-zinc-800 bg-zinc-950 px-3 text-sm font-bold text-zinc-50" value={visionUnit} onChange={(event) => setVisionUnit(event.target.value)}>{['шт','м²','м.п.','тонна','мешок','лист'].map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></label><label className="grid gap-1"><span className="text-xs font-black text-zinc-400">Наличие</span><select className="h-10 rounded-md border border-zinc-800 bg-zinc-950 px-3 text-sm font-bold text-zinc-50" value={visionAvailability} onChange={(event) => setVisionAvailability(event.target.value)}>{['склад','под заказ','нет'].map((item) => <option key={item} value={item}>{item}</option>)}</select></label></div><div className="grid grid-cols-2 gap-2"><Input value={visionCity} onChange={(event) => setVisionCity(event.target.value)} placeholder="Город" /><Input value={visionSupplier} onChange={(event) => setVisionSupplier(event.target.value)} placeholder="Поставщик" /></div><div className="grid grid-cols-[1fr_84px] gap-2"><Input value={visionPurchasePrice} onChange={(event) => setVisionPurchasePrice(event.target.value)} placeholder="Цена закупки" /><select className="h-10 rounded-md border border-zinc-800 bg-zinc-950 px-2 text-sm font-bold text-zinc-50" value={visionPurchaseCurrency} onChange={(event) => setVisionPurchaseCurrency(event.target.value)}><option value="KZT">KZT</option><option value="USD">USD</option></select></div><div className="grid grid-cols-[1fr_84px] gap-2"><Input value={visionSalePrice} onChange={(event) => setVisionSalePrice(event.target.value)} placeholder="Цена продажи" /><select className="h-10 rounded-md border border-zinc-800 bg-zinc-950 px-2 text-sm font-bold text-zinc-50" value={visionSaleCurrency} onChange={(event) => setVisionSaleCurrency(event.target.value)}><option value="KZT">KZT</option><option value="USD">USD</option></select></div><div className="grid grid-cols-2 gap-2"><Input value={visionMarginPercent} onChange={(event) => setVisionMarginPercent(event.target.value)} placeholder="Маржа %" /><select className="h-10 rounded-md border border-zinc-800 bg-zinc-950 px-3 text-sm font-bold text-zinc-50" value={visionStatus} onChange={(event) => setVisionStatus(event.target.value)}>{['draft','ready','published'].map((item) => <option key={item} value={item}>{item}</option>)}</select></div><div className="flex flex-wrap gap-2">{['RU','KK','EN'].map((lang) => <label key={lang} className="flex items-center gap-2 rounded-md border border-zinc-800 px-3 py-2 text-xs font-black text-zinc-300"><input type="checkbox" checked={visionLanguages.includes(lang)} onChange={(event) => setVisionLanguages((items) => event.target.checked ? Array.from(new Set([...items, lang])) : items.filter((item) => item !== lang))} />{lang}</label>)}</div><Input value={visionSeoTitle} onChange={(event) => setVisionSeoTitle(event.target.value)} placeholder="SEO title" /><Textarea className="min-h-20 resize-none text-sm font-bold" value={visionSeoDescription} onChange={(event) => setVisionSeoDescription(event.target.value)} placeholder="SEO description" /><div className="grid gap-2"><Button asChild variant="secondary"><label htmlFor="kazaxbud-material-photo-modal" className="cursor-pointer"><AppIcon name="materials" />Основное фото</label></Button><Input id="kazaxbud-material-photo-modal" className="sr-only" type="file" accept="image/*" onChange={(event) => setVisionFile(event.target.files?.[0] || null)} /><div className="overflow-hidden rounded-md border border-zinc-800 bg-black">{visionPreviewUrl ? <img className="h-40 w-full object-cover" src={visionPreviewUrl} alt="Основное фото для Gemini Vision" /> : <div className="grid h-40 place-items-center text-center text-sm font-bold text-zinc-500">Основное фото</div>}</div><span className="truncate text-xs font-bold text-zinc-500">{visionFile?.name || 'Файл не выбран'}</span></div><div className="grid gap-2"><Button asChild variant="secondary"><label htmlFor="kazaxbud-material-detail-photo-modal" className="cursor-pointer"><AppIcon name="materials" />Детальное фото</label></Button><Input id="kazaxbud-material-detail-photo-modal" className="sr-only" type="file" accept="image/*" onChange={(event) => setVisionDetailFile(event.target.files?.[0] || null)} /><div className="overflow-hidden rounded-md border border-zinc-800 bg-black">{visionDetailPreviewUrl ? <img className="h-32 w-full object-cover" src={visionDetailPreviewUrl} alt="Детальное фото для Gemini Vision" /> : <div className="grid h-32 place-items-center text-center text-sm font-bold text-zinc-500">Второе фото для детализации</div>}</div><span className="truncate text-xs font-bold text-zinc-500">{visionDetailFile?.name || 'Второе фото не выбрано'}</span></div><div className="grid grid-cols-[44px_minmax(0,1fr)_44px] gap-2"><Button variant="secondary" size="icon" type="button" onClick={() => setVisionCount((value) => clampMaterialCount(value - 1))}>-</Button><Input className="text-center text-lg font-black" type="number" min={1} max={12} value={visionCount} onChange={(event) => setVisionCount(clampMaterialCount(Number(event.target.value) || 1))} /><Button variant="secondary" size="icon" type="button" onClick={() => setVisionCount((value) => clampMaterialCount(value + 1))}>+</Button></div><Textarea className="min-h-32 resize-none text-sm font-bold leading-relaxed" value={visionInstruction} onChange={(event) => setVisionInstruction(event.target.value)} /><Button type="button" onClick={() => void runVision()} disabled={visionBusy || !visionFile}><AppIcon name="sparkles" />{visionBusy ? 'Gemini смотрит...' : 'Создать по фото'}</Button><EditorMessage value={visionMessage} />{visionDrafts.length ? <div className="grid gap-3"><div className="flex items-center justify-between gap-3"><strong className="text-sm text-zinc-100">Предпросмотр: {visionDrafts.length}</strong><Button size="sm" type="button" onClick={() => void saveVisionDrafts()} disabled={visionBusy}><AppIcon name="check" />Добавить все</Button></div><div className="grid max-h-72 gap-3 overflow-auto pr-1">{visionDrafts.map((item, index) => <article key={`${item.slug}-${index}`} className="rounded border border-zinc-800 bg-black p-3">{item.imageUrl ? <img className="mb-3 h-24 w-full rounded object-cover" src={item.imageUrl} alt={item.title} /> : null}<div className="mb-2 flex items-start justify-between gap-2"><strong className="text-zinc-100">{item.title}</strong><code className="text-xs text-zinc-500">{item.index}</code></div><p className="text-sm text-zinc-400">{item.text}</p><small className="mt-2 block text-zinc-500">{item.slug} · {item.photo}</small>{item.price ? <small className="mt-1 block text-orange-300">{item.price}</small> : null}</article>)}</div></div> : null}</CardContent></Card>} /> : null}
