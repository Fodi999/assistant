...
export function AffiliatePage({ activeSite }: { activeSite: SiteKey }) {
  ...
  return (
    <section className="ops-page">
      <AdminPageHeader eyebrow="Мультисайтовая админка" title="Партнерка" subtitle="Товары, сети, продавцы, партнерские ссылки, комиссии, cookie days, SEO и офферы." icon="shop" actions={<><DataSourceBadge source={source} label="Товары" /><button className="btn btn-primary" type="button" onClick={() => setEditorProduct(null)}><AppIcon name="sparkles" />Создать партнерский товар</button></>} />
      {sourceError ? <p className="ops-alert"><AppIcon name="terminal" />API не вернул партнерские товары: {sourceError}. Демо-данные отключены.</p> : null}
      ...
      <div className="ops-grid two-one">
        <section className="ops-panel">
          ...
          {rows.length === 0 ? <p className="empty-state">Партнерских товаров из backend нет.</p> : null}
        </section>
        <section className="ops-panel">
