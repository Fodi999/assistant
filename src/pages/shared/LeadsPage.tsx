...
export function LeadsPage({ activeSite }: { activeSite: SiteKey }) {
  ...
  return (
    <section className="ops-page">
      ...
      {sourceError ? <p className="ops-alert"><AppIcon name="terminal" />API не вернул заявки: {sourceError}. Демо-данные отключены.</p> : null}
      ...
      <section className="ops-panel"><table className="ops-table"><thead><tr><th>Клиент</th><th>Источник</th><th>Город</th><th>Сумма</th><th>Статус</th></tr></thead><tbody>{rows.map((lead) => <tr key={lead.id}><td><strong>{lead.clientName}</strong><small>{lead.contact} / {lead.message}</small></td><td>{siteNames[lead.sourceSite]} / {lead.category}</td><td>{lead.city}</td><td>{lead.potentialValue?.toLocaleString('ru-RU')} {lead.currency}</td><td><LeadStatusBadge status={lead.status} /></td></tr>)}</tbody></table>{rows.length === 0 ? <p className="empty-state">Заявок из backend нет.</p> : null}</section>
    </section>
  );
}
