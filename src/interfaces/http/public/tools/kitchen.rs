...
pub async fn scale_recipe(Query(params): Query<ScaleQuery>) -> Json<ScaleResponse> {
    let result = uc::round_to(
        uc::scale(params.value, params.from_portions, params.to_portions),
        2,
    );
    Json(ScaleResponse {
...
pub async fn ingredient_equivalents(
    ...
    let target_units: &[&str] = &[
        "g", "kg", "oz", "lb", "ml", "l", "fl_oz", "tsp", "tbsp", "cup",
    ];
    ...
    let equivalents: Vec<Equivalent> = target_units
        ...
        .filter_map(|&u| {
            uc::convert_with_density(params.value, &params.unit, u, density).map(|v| Equivalent {
                unit: u.to_string(),
                label: label(u, lang),
...
pub async fn food_cost_calc(Query(params): Query<FoodCostQuery>) -> Json<FoodCostResponse> {
    ...
    let price_per_one = if price_amount > 0.0 {
        params.price / price_amount
    } else {
        params.price
    };
    let total_cost = uc::round_to(price_per_one * amount_in_price_unit, 2);

    let cost_per_portion = params
        .portions
        .map(|p| uc::round_to(uc::cost_per_portion(total_cost, p), 2));
...
pub async fn ingredient_suggestions(
    ...
    let suggestions: Vec<Suggestion> = if let Some(ml) = ml_factor {
        let rows: Vec<CatalogNutritionRow> = sqlx::query_as(&format!(
            r#"SELECT {}
...
pub async fn popular_conversions(
    ...
    let db_rows: Vec<CatalogNutritionRow> = sqlx::query_as(&format!(
        r#"SELECT {}
               FROM catalog_ingredients
    ...
    let find_db = |name: &str| -> Option<&CatalogNutritionRow> {
        ...
        db_rows
            ...
            .or_else(|| {
                db_rows
                    .iter()
                    .find(|r| r.name_en.to_lowercase().contains(&n))
            })
    };

    let conversions = POPULAR_CONVERSIONS
        .iter()
        .filter_map(|e| {
