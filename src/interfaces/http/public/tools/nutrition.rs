...
use crate::domain::tools::nutrition::{
    breakdown_per_100g_nullable, macros_ratio, nutrition_score, vitamins_for, MacrosRatio,
    NutritionBreakdownNullable, VitaminData,
};
...
pub async fn nutrition(
    ...
    let (
        ...
        (
            r.localized_name(lang).to_string(),
            r.slug.clone(),
            r.image_url.clone(),
    ...
    let per_100g = breakdown_per_100g_nullable(
...
pub async fn ingredients_db(
    ...
    if !pt_val.is_empty() {
        count_builder = count_builder.bind(&pt_val);
    }
    if !srch_val.is_empty() {
        count_builder = count_builder.bind(&srch_val);
    }
    ...
    if !pt_val.is_empty() {
        row_builder = row_builder.bind(&pt_val);
    }
    if !srch_val.is_empty() {
        row_builder = row_builder.bind(&srch_val);
    }
