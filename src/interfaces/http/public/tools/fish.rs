...
pub async fn fish_season_table(
    ...
    let all_year: Vec<AllYearItem> = all_year_rows
        .iter()
        .map(|r| {
            let name = match lang {
    ...
    let fish: Vec<FishSeasonTableItem> = season_rows
        .iter()
        .map(|r| {
            ...
            let cur_status = pid
                .and_then(|id| {
                    month_rows
                        .iter()
                        .find(|m| m.product_id == id && m.month == cur_month)
                })
                .map(|m| m.status.clone())
                .unwrap_or_else(|| "off".to_string());

            let season: Vec<FishSeasonEntry> = (1u8..=12)
                .map(|m| {
                    let st = pid
                        .and_then(|id| {
                            month_rows
                                .iter()
                                .find(|r| r.product_id == id && r.month == m as i16)
                        })
