...
pub async fn seasonal_calendar(
    ...
    let product_type = params
        .r#type
        .clone()
        .unwrap_or_else(|| "seafood".to_string());
...
pub async fn best_in_season(
    ...
    let month: u8 = params
        .month
        .unwrap_or_else(|| time::OffsetDateTime::now_utc().month() as u8);

    let status_filter = if peak_only {
        "cps.status = 'peak'"
    } else {
        "cps.status IN ('peak','good')"
    };
...
pub async fn recipe_nutrition(
    ...
    for ing in &body.ingredients {
        ...
        let (cal, pro, fat, car) = db
            .map(|r| (r.cal(), r.prot(), r.fat(), r.carbs()))
            .unwrap_or_default();
        ...
        items.push(RecipeIngredientNutrition {
            name: ing.name.clone(),
            localized_name: db
                .map(|r| r.localized_name(lang).to_string())
...
pub async fn recipe_cost(
    ...
    for ing in &body.ingredients {
        ...
        let cost = ing
            .price_per_kg
            .map(|ppkg| uc::round_to((amount_g / 1000.0) * ppkg, 4));
        if cost.is_none() {
    ...
    let cost_per_portion = total_cost
        .zip(portions)
        .filter(|(_, p)| *p > 0.0)
    ...
    let food_cost_percent = cost_per_portion
        .zip(suggested_menu_price)
        .map(|(cpp, smp)| {
            if smp > 0.0 {
                uc::round_to((cpp / smp) * 100.0, 1)
            } else {
                0.0
            }
        });
    ...
    let note = if !all_have_price {
        "Some ingredients are missing price_per_kg — provide prices for full cost calculation."
            .to_string()
    } else {
...
pub async fn best_right_now(
    ...
    let peak: Vec<BestInSeasonItem> = rows
        .iter()
        .filter(|r| r.status == "peak")
        .map(map_row)
        .collect();
    let also_good: Vec<BestInSeasonItem> = rows
        .iter()
    ...
    Json(RegionsResponse {
        regions: vec![
            RegionInfo {
                code: "GLOBAL",
                name_en: "Global",
                description: "Generic global average seasonality",
            },
            RegionInfo {
                code: "PL",
                name_en: "Poland",
                description: "Central European seasonality (Poland)",
            },
            RegionInfo {
                code: "EU",
                name_en: "European Union",
                description: "Average Western/Central Europe",
            },
            RegionInfo {
                code: "ES",
                name_en: "Spain",
                description: "Mediterranean / Southern Europe",
            },
            RegionInfo {
                code: "UA",
                name_en: "Ukraine",
                description: "Eastern European seasonality",
            },
        ],
    })
}
