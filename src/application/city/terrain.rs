...
pub fn terrain_height(x: f32, z: f32, seed: u64) -> f32 {
    ...
    let macro_shape = ((x * 0.004 + s).sin() * 14.0) + ((z * 0.0035 + s * 1.7).cos() * 11.0);
    let mid = ((x * 0.012 + z * 0.008 + s).sin() * 4.8) + ((z * 0.015 - x * 0.009 + s).cos() * 3.2);
    let detail = ((x * 0.05 + z * 0.03 + s).sin() * 0.7) + ((z * 0.06 - x * 0.02 + s).cos() * 0.5);
...
pub fn build_terrain_mesh(width: f32, depth: f32, cell_size: f32, seed: u64) -> CityMesh {
    ...
    for row in 0..=rows {
        for col in 0..=cols {
            ...
            uvs.extend_from_slice(&[col as f32 / cols as f32, row as f32 / rows as f32]);
    ...
    for col in 0..=cols {
        perim.push(0 * stride + col);
    } // front
    for row in 1..=rows {
        perim.push(row * stride + cols);
    } // right
    for col in (0..cols).rev() {
        perim.push(rows * stride + col);
    } // back
    for row in (1..rows).rev() {
        perim.push(row * stride);
    } // left
...
mod tests {
    ...
    fn terrain_mesh_shapes() {
        ...
        assert_eq!(mesh.uvs.len(), expected_total * 2, "uvs len mismatch");
        assert_eq!(mesh.colors.len(), expected_total * 3, "colors len mismatch");
    ...
    fn city_terrain_min_max_consistent() {
        ...
        assert!(
            t.min_height < 0.0,
            "expected some negative terrain, got {}",
            t.min_height
        );
        assert!(
            t.max_height > 0.0,
            "expected some positive terrain, got {}",
            t.max_height
        );
    }
}
