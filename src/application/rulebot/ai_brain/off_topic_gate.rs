...
use super::response_helpers::{borderline_price_suggestions, redirect_suggestions};
...
pub(crate) fn respond_borderline(input: &str, lang: ChatLang) -> ChatResponse {
    ...
    let is_price = t.contains("стоит")
        ...
        || t.contains("коштує")
        || t.contains("почём");
    let is_restaurant = t.contains("ресторан")
        || t.contains("restaurant")
        || t.contains("restauracj")
        || t.contains("кафе")
        || t.contains("cafe");
    let is_delivery = t.contains("доставк")
        || t.contains("deliver")
        || t.contains("dostaw")
        || t.contains("заказать")
        || t.contains("order")
