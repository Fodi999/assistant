...
// ═══════════════════════════════════════════════════════════════════════════════
//  COOKING LOSS — yield / weight change / ужарка / уварка
// ═══════════════════════════════════════════════════════════════════════════════

pub const COOKING_LOSS: &[ScoredKeyword] = &[
    // RU — losses / shrinkage
    ("ужарк",         4), ("уварк",         4), ("усушк",         4),
    ("теряет при",    4), ("теряется при",  4), ("потеря при",    4),
    ("потери при",    4),
    ("при варке",     3), ("при жарке",     3), ("при запекании", 3),
    ("при тушении",   3), ("при готовке",   3), ("при приготовл", 3),
    ("после варки",   3), ("после жарки",   3), ("после готовки", 3),
    ("на пару",       2),
    ("теряет",        2), ("теряется",      2), ("потеря",        2),
    ("потери",        2), ("выход",         2),
    ("сколько остан", 2), ("сколько останется", 2),
    ("усадк",         2), ("ужаривается",   3), ("уваривается",   3),
    // EN
    ("cooking loss",   4), ("weight loss",   3), ("water loss",    3),
    ("cooking yield",  4), ("yield after",   3), ("shrinkage",     4),
    ("lose when cook", 3), ("loses when",    3),
    ("after boiling",  3), ("after frying",  3), ("after cooking", 3),
    ("when boiled",    2), ("when fried",    2), ("when cooked",   2),
    ("how much left",  2),
    // PL
    ("ubytek",         3), ("straty przy",   4), ("traci przy",    4),
    ("po ugotowaniu",  3), ("po usmażeniu",  3), ("po upieczeniu", 3),
    ("uwarzeni",       2), ("wydajność",     3),
    // UK
    ("втрачає при",    4), ("втрати при",    4), ("ужарк",         4),
    ("уварк",          4), ("усушк",         4),
    ("втрачає",        2), ("втрата",        2), ("втрати",        2),
    ("після варіння",  3), ("після смаження",3),
    // Universal
    ("%",              1),
];
