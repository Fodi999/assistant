...
mod tests {
    ...
    #[test]
    fn mod_diet_ru() {
        assert_eq!(detect_modifier("хочу похудеть"), HealthModifier::LowCalorie);
    }
    #[test]
    fn mod_light_ru() {
        assert_eq!(
            detect_modifier("приготовь лёгкое блюдо с треска"),
            HealthModifier::LowCalorie
        );
    }
    #[test]
    fn mod_light_en() {
        assert_eq!(
            detect_modifier("cook a light dish with cod"),
            HealthModifier::LowCalorie
        );
    }
    #[test]
    fn mod_light_uk() {
        assert_eq!(
            detect_modifier("легку страву з тріски"),
            HealthModifier::LowCalorie
        );
    }
    #[test]
    fn mod_sushka() {
        assert_eq!(
            detect_modifier("питание на сушку"),
            HealthModifier::LowCalorie
        );
    }
    ...
    fn mod_mass_ru() {
        assert_eq!(
            detect_modifier("что есть на массу"),
    ...
    fn mod_muscle_en() {
        assert_eq!(
            detect_modifier("high protein foods for muscle"),
            HealthModifier::HighProtein
        );
    }
    #[test]
    fn mod_bulk() {
        assert_eq!(detect_modifier("i'm on bulk"), HealthModifier::HighProtein);
    }

    // ── Quick ──
    #[test]
    fn mod_quick_ru() {
        assert_eq!(detect_modifier("быстрый завтрак"), HealthModifier::Quick);
    }
    #[test]
    fn mod_quick_en() {
        assert_eq!(detect_modifier("quick dinner idea"), HealthModifier::Quick);
    }
    #[test]
    fn mod_fast_pl() {
        assert_eq!(detect_modifier("szybkie danie"), HealthModifier::Quick);
    }
    #[test]
    fn mod_15min() {
        assert_eq!(detect_modifier("рецепт за 15 минут"), HealthModifier::Quick);
    }

    // ── Budget ──
    #[test]
    fn mod_cheap_ru() {
        assert_eq!(detect_modifier("что-то дешевле"), HealthModifier::Budget);
    }
    #[test]
    fn mod_budget_en() {
        assert_eq!(detect_modifier("budget meal ideas"), HealthModifier::Budget);
    }
    #[test]
    fn mod_tanie_pl() {
        assert_eq!(detect_modifier("tanie danie"), HealthModifier::Budget);
    }
    ...
    fn mod_comfort_ru() {
        assert_eq!(
            detect_modifier("что-нибудь сытное"),
            HealthModifier::ComfortFood
        );
    }
    #[test]
    fn mod_comfort_en() {
        assert_eq!(
            detect_modifier("comfort food please"),
            HealthModifier::ComfortFood
        );
    }
    #[test]
    fn mod_hearty() {
        assert_eq!(
            detect_modifier("hearty soup recipe"),
            HealthModifier::ComfortFood
        );
    }
    #[test]
    fn mod_domashn() {
        assert_eq!(
            detect_modifier("домашнее блюдо"),
            HealthModifier::ComfortFood
        );
    }
    ...
    fn mod_fiber_ru() {
        assert_eq!(
            detect_modifier("богато клетчаткой"),
            HealthModifier::HighFiber
        );
    }
    #[test]
    fn mod_fiber_en() {
        assert_eq!(
            detect_modifier("high fiber breakfast"),
            HealthModifier::HighFiber
        );
    }
    #[test]
    fn mod_fiber_pl() {
        assert_eq!(detect_modifier("dużo błonnika"), HealthModifier::HighFiber);
    }

    // ── LowCarb ──
    #[test]
    fn mod_keto_ru() {
        assert_eq!(detect_modifier("кето рецепт"), HealthModifier::LowCarb);
    }
    #[test]
    fn mod_keto_en() {
        assert_eq!(detect_modifier("keto dinner"), HealthModifier::LowCarb);
    }
    #[test]
    fn mod_lowcarb() {
        assert_eq!(detect_modifier("low carb meal"), HealthModifier::LowCarb);
    }
    #[test]
    fn mod_no_carbs() {
        assert_eq!(
            detect_modifier("блюдо без углеводов"),
            HealthModifier::LowCarb
        );
    }
    ...
    #[test]
    fn mod_none() {
        assert_eq!(detect_modifier("приготовь борщ"), HealthModifier::None);
    }
    #[test]
    fn mod_none_en() {
        assert_eq!(detect_modifier("recipe for borscht"), HealthModifier::None);
    }
}
