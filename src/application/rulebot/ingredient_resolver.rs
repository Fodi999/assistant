...
pub async fn resolve_slug(cache: &IngredientCache, hint: &str) -> Option<IngredientData> {
    ...
    // 1. Exact match
    if let Some(p) = cache.get(&h).await {
        return Some(p);
    }
    ...
    if singular != h {
        if let Some(p) = cache.get(singular).await {
            return Some(p);
        }
    }
    if let Some(p) = cache.get(&plural).await {
        return Some(p);
    }
    // -es ending: "tomatoes" → "tomato", "potatoes" → "potato"
    if h.ends_with("es") {
        let stem = &h[..h.len() - 2];
        if let Some(p) = cache.get(stem).await {
            return Some(p);
        }
    }
    ...
    for (from, to) in aliases {
        if h == *from {
            if let Some(p) = cache.get(to).await {
                return Some(p);
            }
        }
    }
