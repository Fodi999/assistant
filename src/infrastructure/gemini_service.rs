use crate::infrastructure::icon_image_prompts::{
    icon_product_mockup_fallback_prompt, icon_product_mockup_prompt,
};
...
- when the reference product contract is active: people, hands, chefs, preparation actions, added food, changed assortment, changed quantity or obstructed product"#,
    ...
    pub async fn generate_icon_product_mockup_image(
        ...
        let icon_mockup_model = std::env::var("GEMINI_ICON_MOCKUP_IMAGE_MODEL")
            .unwrap_or_else(|_| "gemini-3.1-flash-image".to_string());
        let prompt = icon_product_mockup_prompt(scene, scale_direction, reference_urls.len());
        let fallback_prompt = icon_product_mockup_fallback_prompt(scene);
        let result = self
            .generate_image_from_prompt_with_references(
                &prompt,
                article_title,
