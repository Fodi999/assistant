...
use crate::domain::tools::nutrition::{self, MacrosRatio, NutritionBreakdown};
...
impl DietFlags {
    ...
    pub fn active_labels(&self) -> Vec<&'static str> {
        ...
        if self.mediterranean {
            labels.push("mediterranean");
        }
        if self.low_carb {
            labels.push("low_carb");
        }
        labels
    }
}
...
pub fn analyze_recipe(ingredients: &[RecipeIngredientInput], portions: u32) -> RecipeAnalysis {
    ...
    // ── 4. Flavor analysis ──
    let flavor_ingredients: Vec<FlavorIngredient> = ingredients
        .iter()
        .map(|i| FlavorIngredient {
            slug: i.slug.clone(),
            grams: i.grams,
...
mod tests {
    ...
    fn recipe_analysis_basic() {
        ...
        assert!(
            result.per_portion.calories > 200.0,
            "per portion cal > 200, got {}",
            result.per_portion.calories
        );
        assert!(
            result.per_portion.calories < 300.0,
            "per portion cal < 300, got {}",
            result.per_portion.calories
        );
