...
pub fn diagnose(ctx: &RecipeContext) -> RuleDiagnosis {
    ...
    let category_scores = CategoryScores {
        ...
        flavor: ((flavor_rule * 0.5 + balance * 0.5)
            .clamp(20.0, 100.0)
            .round()) as u8,
        // nutrition: 50% rule-based + 50% actual nutrition_score
        nutrition: ((nutrition_rule * 0.5 + nutrition * 0.5)
            .clamp(20.0, 100.0)
            .round()) as u8,
...
fn check_nutrition_rules(ctx: &RecipeContext, issues: &mut Vec<RuleIssue>) {
    ...
    if ctx.protein_pct < 12.0 {
        let sev = if ctx.protein_pct < 8.0 {
            "critical"
        } else {
            "warning"
        };
...
fn check_structure_rules(ctx: &RecipeContext, issues: &mut Vec<RuleIssue>) {
    ...
    let has_protein =
        has("meat") || has("fish") || has("seafood") || has("egg") || has("dairy") || has("legume");
    ...
    let has_acid =
        types.iter().any(|&pt| pt == "fruit" || pt == "vegetable") && ctx.flavor.acidity > 2.0;
...
mod tests {
    ...
    fn no_contradiction_high_fat_and_low_fat() {
        ...
        assert!(
            rules.contains(&"high_fat_ratio"),
            "high_fat_ratio should fire"
        );
        assert!(
            !rules.contains(&"low_fat"),
            "low_fat must be suppressed when high_fat_ratio fires"
        );
        assert!(
            !rules.contains(&"missing_fat_source"),
            "missing_fat_source must be suppressed when high_fat_ratio fires"
        );
    }
    ...
    fn no_contradiction_high_sugar_and_low_sweetness() {
        ...
        assert!(
            !rules.contains(&"low_sweetness"),
            "low_sweetness must be suppressed when high_sugar fires"
        );
    }
    ...
    fn low_fat_fires_when_no_high_fat_ratio() {
        ...
        assert!(
            !rules.contains(&"high_fat_ratio"),
            "high_fat_ratio should NOT fire at 25%"
        );
        assert!(
            rules.contains(&"low_fat"),
            "low_fat should fire when not contradicted"
        );
    }
}
