...
impl ToolId {
    ...
    pub fn engine(&self) -> EngineKind {
        use ToolId::*;
        match self {
            Convert | ListUnits | IngredientScale | IngredientConvert | SeoIngredientConvert
            | MeasureConversion | IngredientMeasures => EngineKind::Conversion,

            Nutrition | IngredientsDb | CompareFoods | ResolveSlug => EngineKind::Nutrition,

            FishSeason | FishSeasonTable | SeasonalCalendar | InSeasonNow | ProductSeasonality
            | BestInSeason | ProductsByMonth | BestRightNow | ProductSearch | ListRegions => {
            ...
            | FoodCost
            | IngredientSuggestions
            | PopularConversions => EngineKind::Kitchen,
    ...
    pub fn cache_ttl_secs(&self) -> u64 {
        ...
        match self {
            ...
            FishSeason | FishSeasonTable | SeasonalCalendar | InSeasonNow | ProductSeasonality
            | BestInSeason | ProductsByMonth | BestRightNow | ListRegions => 86_400,

            // Nutrition / catalog — 1 hour
            Nutrition | IngredientsDb | CompareFoods | ResolveSlug | IngredientMeasures
            | MeasureConversion | ProductSearch => 3_600,

            // Static conversions — 7 days
            Convert | ListUnits | PopularConversions => 604_800,

            // Dynamic / user-specific — no cache
            RecipeAnalyze | RecipeNutrition | RecipeCost | ShareRecipe | GetSharedRecipe => 0,
