-- Delete junk test/alert categories that remain from load testing
-- Products in these categories were already soft-deleted and now need hard-delete
-- before we can remove the categories

-- Hard-delete any remaining soft-deleted ingredients in junk test categories
...
-- Delete the junk Test Cat / Alert Cat categories
...
-- Also clean up any remaining junk ingredients by name pattern
DELETE FROM catalog_ingredients
WHERE deleted = true
   OR name_en LIKE 'Test %'
   OR name_en LIKE 'Expiring Fish%'
   OR name_en LIKE 'Low Milk%'
   OR name_en LIKE '%\_964' ESCAPE '\'
   OR name_en LIKE '%\_133' ESCAPE '\';
