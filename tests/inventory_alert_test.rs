use restaurant_backend::application::{InventoryAlertService, InventoryService};
...
async fn test_inventory_alerts_suite() {
    ...
    inventory_service
        .add_batch(
            user_id,
            tenant_id,
            CatalogIngredientId::from_uuid(ing_exp_id),
            1000,
            10.0,
            None,
            None,
    ...
    let exp_alerts: Vec<_> = alerts
        .iter()
        .filter(|a| a.alert_type == InventoryAlertType::ExpiringBatch)
        .collect();

    for a in &exp_alerts {
        println!(
            "Found aggregated expiration alert: {} ({:?})",
    ...
    assert!(exp_alerts
        .iter()
        .any(|a| a.severity == AlertSeverity::Critical && a.ingredient_id == ing_exp_id));
    ...
    let milk_alert_0 = alerts_before
        .iter()
        .find(|a| a.ingredient_id == ing_low_id)
        .expect("Should have out of stock alert");
    ...
    inventory_service
        .add_batch(
            user_id,
            tenant_id,
            CatalogIngredientId::from_uuid(ing_low_id),
            500,
            3.0,
            None,
            None,
    ...
    let milk_alert_3 = alerts_after
        .iter()
        .find(|a| a.ingredient_id == ing_low_id)
        .expect("Should have low stock alert");
    ...
    assert!(
        !has_milk_alert,
        "Should not have low stock alert when above threshold"
    );
