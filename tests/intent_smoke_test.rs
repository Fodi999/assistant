...
use restaurant_backend::application::rulebot::intent_router::{
    detect_intent, detect_intent_scored, detect_language, parse_input, ChatLang, Intent,
};
...
fn check(label: &str, input: &str, expected_intent: Intent) {
    ...
    let status = if scored.intent == expected_intent {
        "✅"
    } else {
        "❌"
    };
...
fn test_weight_loss_4_languages() {
    ...
    check(
        "EN weight loss+recipe",
        "make a light soup for weight loss",
        Intent::RecipeHelp,
    );
    check(
        "PL odchudzanie+przepis",
        "ugotuj lekką zupę na odchudzanie",
        Intent::RecipeHelp,
    );
    check(
        "UK схуднення+рецепт",
        "приготуй легкий суп для схуднення",
        Intent::RecipeHelp,
    );
    ...
    check(
        "RU need",
        "хочу похудеть, но нужно много белка",
        Intent::MealIdea,
    );
    check(
        "EN need",
        "want to lose weight but need high protein",
        Intent::MealIdea,
    );
    check(
        "PL need",
        "chcę schudnąć, ale potrzebuję dużo białka",
        Intent::MealIdea,
    );
    check(
        "UK need",
        "хочу схуднути, але потрібно багато білка",
        Intent::MealIdea,
    );
}
...
fn test_muscle_gain_4_languages() {
    ...
    check(
        "RU масса+рецепт",
        "приготовь высокобелковый салат на массу",
        Intent::RecipeHelp,
    );
    check(
        "EN muscle+recipe",
    ...
    check(
        "PL masa+przepis",
        "przygotuj sałatkę wysokobiałkową na masę",
        Intent::RecipeHelp,
    );
    check(
        "UK маса+рецепт",
        "приготуй високобілковий салат на масу",
        Intent::RecipeHelp,
    );
    ...
    check(
        "PL masa+meal",
        "co ugotować na masę na obiad?",
        Intent::MealIdea,
    );
    check(
        "UK маса+meal",
        "що приготувати на масу на вечерю?",
        Intent::MealIdea,
    );
}
...
fn test_gluten_free_4_languages() {
    ...
    check(
        "RU рецепт без глютена",
        "рецепт без глютена",
        Intent::RecipeHelp,
    );
    check(
        "EN gluten free recipe",
        "gluten free recipe",
        Intent::RecipeHelp,
    );
    check(
    ...
    check(
        "RU что без глютена",
        "что приготовить без глютена на ужин?",
        Intent::MealIdea,
    );
    check(
        "EN what gluten free",
        "what to cook gluten free for dinner?",
        Intent::MealIdea,
    );
    check(
        "PL co bezglutenowe",
        "co ugotować bezglutenowe na kolację?",
        Intent::MealIdea,
    );
    check(
        "UK що без глютену",
        "що приготувати без глютену на вечерю?",
        Intent::MealIdea,
    );
}
