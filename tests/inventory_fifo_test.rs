...
async fn test_inventory_fifo_full_suite() {
    ...
    let batch1_id = inventory_service
        .add_batch(
            ...
            1000,
            10.0,
            Some("Supplier A".to_string()),
            Some("INV-001".to_string()),
            now - time::Duration::days(1),
            expiry1,
        )
        .await
        .expect("Failed to add batch 1");
    ...
    let batch2_id = inventory_service
        ...
        .await
        .expect("Failed to add batch 2");

    let batches = inventory_service
        .list_products(user_id, tenant_id)
        .await
        .expect("Failed to list");
    // Filter by catalog_id to be safe
    let b1 = batches
        .iter()
        .find(|b| b.id == batch1_id)
        .expect("Batch 1 not found");
    let b2 = batches
        .iter()
        .find(|b| b.id == batch2_id)
        .expect("Batch 2 not found");
    ...
    inventory_service
        .deduct_fifo(tenant_id, catalog_id, 3.0, None, None, None)
        .await
        .expect("Deduction failed");

    let b1_after = inventory_service
        .get_product(user_id, tenant_id, batch1_id)
        .await
        .unwrap()
        .unwrap();
    ...
    inventory_service
        .deduct_fifo(tenant_id, catalog_id, 9.0, None, None, None)
        .await
        .expect("Deduction failed");

    let b1_final = inventory_service
        .get_product(user_id, tenant_id, batch1_id)
        .await
        .unwrap()
        .unwrap();
    let b2_intermediate = inventory_service
        .get_product(user_id, tenant_id, batch2_id)
        .await
        .unwrap()
        .unwrap();
    ...
    inventory_service
        ...
        .await
        .expect("Deduction failed");
    let b2_final = inventory_service
        .get_product(user_id, tenant_id, batch2_id)
        .await
        .unwrap()
        .unwrap();
    ...
    let batch3_id = inventory_service
        .add_batch(
            user_id,
            tenant_id,
            catalog_id,
        ...
        .await
        .expect("Failed to add batch 3");

    let batch4_id = inventory_service
        .add_batch(
            user_id,
            tenant_id,
            catalog_id,
        ...
        .await
        .expect("Failed to add batch 4");

    // Total is exactly 2.0
    inventory_service
        .deduct_fifo(tenant_id, catalog_id, 2.0, None, None, None)
        .await
        .expect("Deduction failed");

    let b3 = inventory_service
        .get_product(user_id, tenant_id, batch3_id)
        .await
        .unwrap()
        .unwrap();
    let b4 = inventory_service
        .get_product(user_id, tenant_id, batch4_id)
        .await
        .unwrap()
        .unwrap();
    ...
    let batch5_id = inventory_service
        .add_batch(
            user_id,
            tenant_id,
            catalog_id,
    ...
    assert_eq!(
        failures, 1,
        "One deduction should fail due to insufficient stock (10 - 6 - 6 = -2)"
    );

    let b5 = inventory_service
        .get_product(user_id, tenant_id, batch5_id)
        .await
        .unwrap()
        .unwrap();
    ...
    let batch_b_id = inventory_service
        .add_batch(
            user_id,
            tenant_b_id,
            catalog_id,
        ...
        .await
        .expect("Failed to add batch for Tenant B");
    ...
    let result_a = inventory_service
        .deduct_fifo(tenant_id, catalog_id, 5.0, None, None, None)
        .await;
    assert!(result_a.is_err(), "Tenant A should not have enough stock");

    // Tenant B should still have 20.0
    let b_stock = inventory_service
        .get_product(user_id, tenant_b_id, batch_b_id)
        .await
        .unwrap()
        .unwrap();
