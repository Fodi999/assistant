...
2. **Сортировка FIFO**: Фронтенд больше не должен заботиться о том, какую партию списывать. Просто отправляйте запрос на списание нужного количества, бэкенд сам выберет сначала то, что испортится раньше.
3. **Обновление данных**: После любого изменения инвентаря (добавление поставки или продажа), желательно инвалидировать кэш `inventory/health`, чтобы `badge_count` в шапке обновился актуально.
